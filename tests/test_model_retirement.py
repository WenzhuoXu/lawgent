...
def test_retired_model_resolves_to_the_configured_default():
    ...
    assert settings.resolve_model_for_provider("gpt-5.5", "openai") == "gpt-5.6-terra"
...
def test_fast_tier_is_not_promoted_to_the_high_effort_default():
    ...
    settings = current_settings()
    assert settings.resolve_model_for_provider("gpt-5.6-luna", "openai") == "gpt-5.6-luna"
...
def test_pinned_config_model_counts_as_offered(monkeypatch):
    ...
    assert settings.resolve_model_for_provider("gpt-5.6-sol", "openai") == "gpt-5.6-sol"
    cfg._CACHED = None


def test_cross_provider_and_empty_selections_fall_back():
    settings = current_settings()
    assert settings.resolve_model_for_provider("claude-opus-5", "openai") == "gpt-5.6-terra"
    assert settings.resolve_model_for_provider("", "openai") == "gpt-5.6-terra"
    assert settings.resolve_model_for_provider(None, "openai") == "gpt-5.6-terra"


def test_anthropic_selections_resolve_against_their_own_tier():
    settings = current_settings()
    assert (
        settings.resolve_model_for_provider("claude-opus-4-7", "anthropic") == "claude-opus-4-7"
    )
...
def test_store_normalizes_an_incoming_retired_selection(tmp_path):
    ...
    assert chat.settings.model == "gpt-5.6-terra"
    ...
    assert patched.settings.model == "gpt-5.6-terra"
    assert store.get_chat(chat.id).settings.model == "gpt-5.6-terra"
...
def test_existing_rows_are_migrated_when_the_store_opens(tmp_path):
    ...
    keep = store.create_chat(settings=ChatSettings(provider="openai", model="gpt-5.6-luna"))
    stale = store.create_chat(settings=ChatSettings(provider="openai", model="gpt-5.6-terra"))
    ...
    con.execute(
        ...
        (json.dumps({"provider": "openai", "model": "gpt-5.5"}), stale.id),
    )
    con.commit()
    con.close()

    reopened = ChatStore(db_path=db)
    assert reopened.get_chat(stale.id).settings.model == "gpt-5.6-terra"
    assert reopened.get_chat(keep.id).settings.model == "gpt-5.6-luna"
...
def test_run_settings_resolve_a_retired_chat_selection():
    ...
    run_settings = _settings_for_chat(ChatSettings(provider="openai", model="gpt-5.5"))
    assert run_settings.openai_model == "gpt-5.6-terra"
    assert run_settings.model_for_provider() == "gpt-5.6-terra"
